...
module.exports = async (req, res) => {
  logRegister(`Incoming request: ${JSON.stringify(req.body)}`);
  ...
  try {
    ...
    if (!isAppSignatureValid) {
      logRegister('Unauthorized app attempt.');
    ...
    if (!userSnapshot.empty) {
      logRegister('Attempt to register with an existing username.');
    ...
    logRegister('User registered successfully.');
    return res.status(201).json({ message: 'User registered successfully.' });
  } catch (error) {
    logRegister(`Registration failed: ${error.message}`);
